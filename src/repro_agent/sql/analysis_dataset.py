...
def build_tyg_stroke_analysis_dataset_sql(
    mode: str,
    has_sepsis3_flag: bool,
    profile: TygSepsisCohortProfile = PAPER_MIMIC_TYG_STROKE_PROFILE,
) -> str:
    cohort_sql = build_tyg_stroke_cohort_sql(mode=mode, has_sepsis3_flag=has_sepsis3_flag, profile=profile)
    return f"""
WITH cohort AS (
{_indent_sql(cohort_sql, 4)}
),
...
dx_flags AS (
    SELECT
        c.hadm_id,
        MAX(CASE WHEN {_diagnosis_match_sql('dx', COMORBIDITY_CODE_PREFIXES['hypertension'])} THEN 1 ELSE 0 END) AS hypertension
    FROM cohort_ctx c
    LEFT JOIN mimiciv_hosp.diagnoses_icd dx
        ON dx.hadm_id = c.hadm_id
    GROUP BY c.hadm_id
...
charlson_flags AS (
    SELECT
        c.hadm_id,
        COALESCE(ch.myocardial_infarct, 0) AS myocardial_infarction,
        COALESCE(ch.congestive_heart_failure, 0) AS congestive_heart_failure,
        COALESCE(ch.chronic_pulmonary_disease, 0) AS chronic_pulmonary_disease,
...
glucose_labs AS (
    SELECT
        c.hadm_id,
        MIN(le.valuenum) FILTER (WHERE le.valuenum IS NOT NULL) AS min_glucose,
        MAX(le.valuenum) FILTER (WHERE le.valuenum IS NOT NULL) AS max_glucose
    FROM cohort_ctx c
    LEFT JOIN mimiciv_hosp.labevents le
        ON le.hadm_id = c.hadm_id
       AND le.itemid IN ({_int_list(GLUCOSE_ITEMIDS)})
       AND le.charttime >= {_lab_window_lower_bound_sql("c", profile)}
       AND le.charttime <= {_lab_window_upper_bound_sql("c", profile)}
    GROUP BY c.hadm_id
),
height_weight AS (
    SELECT
        c.stay_id,
        round(h.height::numeric, 3) AS height_cm,
        round(w.weight::numeric, 3) AS weight_kg
    FROM cohort_ctx c
...
iv_tpa_rx AS (
    ...
    WHERE LOWER(COALESCE(rx.drug, '')) LIKE '%alteplase%'
      AND LOWER(COALESCE(rx.drug, '')) NOT LIKE '%catheter clearance%'
      AND LOWER(COALESCE(rx.drug, '')) NOT LIKE '%external ventricular drain%'
      AND COALESCE(rx.starttime, c.intime) <= c.outtime
      AND COALESCE(rx.stoptime, rx.starttime, c.outtime) >= c.intime
),
iv_tpa_proc AS (
    SELECT DISTINCT
        c.hadm_id,
    ...
    JOIN mimiciv_hosp.d_icd_procedures dp
        ON dp.icd_code = p.icd_code
       AND dp.icd_version = p.icd_version
    WHERE (
        (p.icd_version = 9 AND p.icd_code = '9910')
        OR (p.icd_version = 10 AND p.icd_code LIKE '3E0%17')
        OR LOWER(COALESCE(dp.long_title, '')) LIKE '%thrombolytic%'
    )
),
mechanical_thrombectomy AS (
    SELECT DISTINCT
        c.hadm_id,
        1 AS mechanical_thrombectomy
    FROM cohort_ctx c
    JOIN mimiciv_hosp.procedures_icd p
        ON p.hadm_id = c.hadm_id
    JOIN mimiciv_hosp.d_icd_procedures dp
       ...
       AND dp.icd_version = p.icd_version
    WHERE (
        (p.icd_version = 9 AND p.icd_code IN ('0062', '1754'))
        OR (
            p.icd_version = 10
            AND LOWER(COALESCE(dp.long_title, '')) LIKE 'extirpation of matter from intracranial artery%'
            AND (
                LOWER(COALESCE(dp.long_title, '')) LIKE '%percutaneous%'
                OR LOWER(COALESCE(dp.long_title, '')) LIKE '%stent retriever%'
            )
        )
    )
),
insulin_treatment AS (
    SELECT DISTINCT
        c.stay_id,
        1 AS insulin_treatment
    FROM cohort_ctx c
    JOIN mimiciv_hosp.prescriptions rx
        ON rx.hadm_id = c.hadm_id
    WHERE LOWER(COALESCE(rx.drug, '')) LIKE '%insulin%'
      AND LOWER(COALESCE(rx.drug, '')) NOT LIKE '%catheter clearance%'
      AND LOWER(COALESCE(rx.drug, '')) NOT LIKE '%external ventricular drain%'
      AND COALESCE(rx.starttime, c.intime) <= c.outtime
      AND COALESCE(rx.stoptime, rx.starttime, c.outtime) >= c.intime
...
SELECT
    ...
    CASE
        ...
        THEN round((hw.weight_kg / power(hw.height_cm / 100.0, 2))::numeric, 6)
        ELSE NULL
    END AS bmi,
    COALESCE(dx.hypertension, 0) AS hypertension,
    COALESCE(ch.myocardial_infarction, 0) AS myocardial_infarction,
    COALESCE(ch.congestive_heart_failure, 0) AS congestive_heart_failure,
    COALESCE(ch.chronic_pulmonary_disease, 0) AS chronic_pulmonary_disease,
    COALESCE(ch.peripheral_vascular_disease, 0) AS peripheral_vascular_disease,
    COALESCE(ch.renal_disease, 0) AS renal_disease,
    round(ch.charlson_score::numeric, 6) AS charlson_score,
    round(vs.heart_rate_mean::numeric, 6) AS heart_rate,
    round(vs.sbp_mean::numeric, 6) AS sbp,
    round(vs.dbp_mean::numeric, 6) AS dbp,
    round(vs.mbp_mean::numeric, 6) AS mbp,
    round(vs.spo2_mean::numeric, 6) AS spo2,
    round(vs.temperature_mean::numeric, 6) AS temperature,
    round(cbc.wbc::numeric, 6) AS white_blood_cell_count,
    round(cbc.platelet::numeric, 6) AS platelet_count,
    round(cbc.hemoglobin::numeric, 6) AS hemoglobin,
    round(chem.creatinine::numeric, 6) AS creatinine,
    round(chem.bun::numeric, 6) AS bun,
    round(fd_gcs.gcs_min::numeric, 6) AS gcs_score,
    round(fd_sofa.sofa::numeric, 6) AS sofa_score,
    round(saps.sapsii::numeric, 6) AS saps_ii,
    round(aps.apsiii::numeric, 6) AS apsiii,
    round(oasis.oasis::numeric, 6) AS oasis,
    round(sirs.sirs::numeric, 6) AS sirs,
    round(COALESCE(c.blood_glucose, chem.chemistry_glucose, glucose_labs.min_glucose)::numeric, 6) AS initial_glucose,
    round(COALESCE(glucose_labs.max_glucose, vs.glucose_max, chem.chemistry_glucose)::numeric, 6) AS max_glucose,
    round(c.triglycerides::numeric, 6) AS triglycerides,
    round(c.blood_glucose::numeric, 6) AS blood_glucose,
    round(c.tyg_index::numeric, 6) AS tyg_index,
    c.tyg_quartile,
    CASE WHEN COALESCE(tpa_rx.iv_tpa, 0) = 1 OR COALESCE(tpa_proc.iv_tpa, 0) = 1 THEN 1 ELSE 0 END AS iv_tpa,
    CASE WHEN thrombectomy.mechanical_thrombectomy IS NULL THEN 0 ELSE 1 END AS mechanical_thrombectomy,
    CASE WHEN insulin.insulin_treatment IS NULL THEN 0 ELSE 1 END AS insulin_treatment,
    c.icu_mortality,
    c.in_hospital_mortality,
    c.mortality_30d,
    c.mortality_90d,
    c.mortality_180d,
    c.mortality_1y,
    round(c.time_to_icu_event_days::numeric, 6) AS time_to_icu_event_days,
    round(c.time_to_in_hospital_event_days::numeric, 6) AS time_to_in_hospital_event_days,
    round(c.time_to_event_30d_days::numeric, 6) AS time_to_event_30d_days,
    round(c.time_to_event_90d_days::numeric, 6) AS time_to_event_90d_days,
    round(c.time_to_event_180d_days::numeric, 6) AS time_to_event_180d_days,
    round(c.time_to_event_1y_days::numeric, 6) AS time_to_event_1y_days,
    round((c.icu_los_hours / 24.0)::numeric, 6) AS icu_los_days,
    round((c.hospital_los_hours / 24.0)::numeric, 6) AS hospital_los_days
FROM cohort_ctx c
LEFT JOIN dx_flags dx
    ON dx.hadm_id = c.hadm_id
LEFT JOIN charlson_flags ch
    ON ch.hadm_id = c.hadm_id
LEFT JOIN mimiciv_derived.first_day_vitalsign vs
    ON vs.stay_id = c.stay_id
LEFT JOIN mimiciv_derived.first_day_gcs fd_gcs
...
LEFT JOIN chem
    ON chem.hadm_id = c.hadm_id
LEFT JOIN glucose_labs
    ON glucose_labs.hadm_id = c.hadm_id
LEFT JOIN height_weight hw
    ON hw.stay_id = c.stay_id
LEFT JOIN iv_tpa_rx tpa_rx
    ON tpa_rx.hadm_id = c.hadm_id
LEFT JOIN iv_tpa_proc tpa_proc
    ON tpa_proc.hadm_id = c.hadm_id
LEFT JOIN mechanical_thrombectomy thrombectomy
    ON thrombectomy.hadm_id = c.hadm_id
LEFT JOIN insulin_treatment insulin
    ON insulin.stay_id = c.stay_id
ORDER BY c.stay_id
"""


def build_hr_trajectory_analysis_dataset_sql(
    mode: str,
    has_sepsis3_flag: bool,
    profile: TygSepsisCohortProfile = PAPER_MIMIC_HEART_RATE_TRAJECTORY_PROFILE,
) -> str:
    cohort_sql = build_hr_trajectory_sepsis_cohort_sql(mode=mode, has_sepsis3_flag=has_sepsis3_flag, profile=profile)
...
temp_raw AS (
    ...
    WHERE v.charttime >= {_lab_window_lower_bound_sql("c", profile)}
      AND v.charttime <= {_lab_window_upper_bound_sql("c", profile)}
...
SELECT
    ...
    CASE
        ...
        ELSE NULL
    END AS bmi,
    round(COALESCE(fd_sofa.sofa, c.sofa_score)::numeric, 6) AS sofa_score,
    round(aps.apsiii::numeric, 6) AS apsiii,
    NULL::double precision AS gcs_score,
    round(temp.temperature::numeric, 6) AS temperature,
    round(cbc.hemoglobin::numeric, 6) AS hemoglobin,
    ...
    CASE WHEN rrt_flag.renal_replacement_therapy IS NULL THEN 0 ELSE 1 END AS renal_replacement_therapy,
    CASE WHEN vasopressor_flag.vasopressor_use IS NULL THEN 0 ELSE 1 END AS vasopressor_use,
    NULL::int AS beta_blocker_use,
    round(c.heart_rate_hour_1::numeric, 6) AS heart_rate_initial,
    round((({_trajectory_mean_sum_sql(hours)}) / {hours})::numeric, 6) AS heart_rate_mean_10h,
    round((c.heart_rate_hour_{hours} - c.heart_rate_hour_1)::numeric, 6) AS heart_rate_delta_10h,
    {_trajectory_projection_select_sql(hours)}
    c.sepsis3_flag,
    c.suspected_infection_time,
    c.sofa_time,
    c.first_heart_rate_charttime,
    c.hourly_measurement_count,
    c.mortality_30d,
    round(c.time_to_event_30d_days::numeric, 6) AS time_to_event_30d_days,
    round((c.icu_los_hours / 24.0)::numeric, 6) AS icu_los_days,
    round((c.hospital_los_hours / 24.0)::numeric, 6) AS hospital_los_days
...
def _trajectory_mean_sum_sql(hours: int) -> str:
    return " + ".join(f"c.heart_rate_hour_{hour}" for hour in range(1, hours + 1))


def _trajectory_projection_select_sql(hours: int) -> str:
    return "".join(
        f"round(c.heart_rate_hour_{hour}::numeric, 6) AS heart_rate_hour_{hour},\n    "

...
class PaperPreset:
    ...
    def matches(self, haystack: str) -> bool:
        ...
        return (
            all(term.lower() in lowered for term in self.required_terms)
            and not any(term.lower() in lowered for term in self.exclusion_terms)
        )
...
MIMIC_TYG_SEPSIS_PRESET = PaperPreset(
    ...
    exclusion_terms=("ischemic stroke",),
    notes=(
